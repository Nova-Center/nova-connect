...
export function RegisterForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      formData.append("birthDate", values.birth_date.toISOString().split("T")[0])
      ...
      const response = await axios.post("http://localhost:3333/api/v1/auth/register", formData)
      ...
      form.setError("email", {
        message:
          data?.message ||
          (Array.isArray(data?.errors) ? data.errors[0]?.message : "Erreur lors de l'inscription."),
      })
    }
    finally {
