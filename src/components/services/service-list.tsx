...
export default function ServiceList({
  ...
  if (status === 'unauthenticated' || !user?.accessToken) {
    return (
      <div className="flex items-center justify-center p-12">
        <div className="text-center space-y-3">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            {searchQuery
              ...
              : 'Soyez le premier à proposer un service !'}
          </p>
        </div>
      </div>
    );
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {isParticipant && !isOwner && (
                    <UserMinus className="h-4 w-4 text-green-800" />
                  )}
                </div>
              </div>
              {/* Description */}
              ...
              <div className="flex gap-2 pt-2">
                {isOwner ? (
                  ...
                  <Button
                    ...
                    className="w-full"
                  >
                    <UserMinus className="h-4 w-4 mr-2" /> Annuler
                  </Button>
                ) : hasVolunteer ? (
                  <Button disabled className="w-full" variant="destructive">
                    <UserMinus className="h-4 w-4 mr-2" /> Déjà réservé
                  </Button>
                ) : (
                  <Button
                    onClick={() => handleParticipate(service.id)}
                    className="w-full"
                  >
                    <UserPlus className="h-4 w-4 mr-2" /> Participer
                  </Button>
                )}
              </div>
            </CardContent>
