...
export default function ChatInterface() {
  ...
  // Charger le fil + compteur non lus
  useEffect(() => {
    if (!token) return
    const load = async () => {
      try {
        const [convRes, countRes] = await Promise.all([
          axios.get<Message[]>(
            `${API}/api/v1/messages/conversation/${otherUserId}`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
          axios.get<{ count: number }>(
            `${API}/api/v1/messages/unread-count`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
        ])
  ...
  // Marquer tous comme lus (pour cette conversation)
  const markAllAsRead = async () => {
    if (!token) return
    try {
      await axios.post(
        `${API}/api/v1/messages/mark-as-read/${otherUserId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      )
